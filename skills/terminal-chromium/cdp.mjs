#!/usr/bin/env node
// CDP (Chrome DevTools Protocol) client for terminal-chromium
// Usage: CDP_PORT=9222 cdp.mjs <command> [target] [args...]
      ...
      case 'snap': {
        ...
        const print = (node, depth = 0) => {
          ...
          if (node.children) node.children.forEach(c => {
            const child = nodes.find(n => n.nodeId === c.nodeId) || c;
            print(child, depth + 1);
          });
        };
        if (nodes.length) print(nodes[0]);
        break;
      }
      case 'shot': {
